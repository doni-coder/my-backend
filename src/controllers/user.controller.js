...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const avatar = await uploadOnColudinary(avatarLocalPth);

  if (!avatar.url) {
    throw new ApiError(400, "Error while uploading avatara");
  }
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  const coverImageLocalPth = req.file?.path;
  if (!coverImageLocalPth) {
    throw new ApiError(400, "CoverImage file is missing");
  }

  const coverImage = await uploadOnColudinary(coverImageLocalPth);
